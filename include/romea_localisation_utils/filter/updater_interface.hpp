...
class UpdaterInterface : public UpdaterInterfaceBase {
 ...
 public:
  UpdaterInterface(std::shared_ptr<rclcpp::Node> node,
...
UpdaterInterface<Filter_, Updater_, Msg>::UpdaterInterface(
  ...
  auto callback = std::bind(&UpdaterInterface::process_message, this,
                            std::placeholders::_1);
...
void UpdaterInterface<Filter_, Updater_, Msg>::process_message(
  ...
  auto update_function = std::bind(
      &Updater::update, updater_.get(), std::placeholders::_1,
      std::move(observation), std::placeholders::_2, std::placeholders::_3);
